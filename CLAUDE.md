...
**Multi-pick = one `GroupEntry`** (`storage.ts`, `HistoryEntry = SingleEntry | GroupEntry`, `isGroupEntry` checks `picks`). Content mints `sessionId` on `MULTI_PICK_START` (not `crypto.randomUUID` — secure-context only, content runs on http pages), sends it on every multi-pick `ELEMENT_SELECTED`. `addToGroup` creates the group on first pick (empty session → no entry), appends, moves it to front; one group = one `MAX_HISTORY` slot. `MAX_GROUP_PICKS` (25): content counts picks and deactivates at the cap; `addToGroup` throws past it (unreachable). Popup group row: expand → per-pick copyable rows; "Copy all" → `toLocatorList` (`shared/page-object.ts`): `<locator>; // <name>` per pick, names from last `getBy*`/`locator` call's args + role/tag suffix, deduped with numeric suffix. 25-pick cap = manual check only (no E2E).
...
- **Settings** (`src/shared/settings.ts`): `chrome.storage.local` key `pickwright_settings`, read by background + popup + content. `copyOnPick` (default on): content copies the picked locator to the clipboard; off → toast says `Picked`, not `Copied`. `historyMode`: `keep` / `autoClear` (wipe on browser startup) / `off` (never record). `getSettings` merges over `DEFAULT_SETTINGS`.
